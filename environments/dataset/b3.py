...
top_ten_stocks = {
    '2017': ['ITUB4', 'BBDC4', 'ABEV3', 'PETR4', 'VALE3', 'BRFS3', 'BBAS3', 'ITSA4', 'B3SA3', 'UGPA3'],
    '2018': ['ITUB4', 'VALE3', 'BBDC4', 'ABEV3', 'PETR4', 'B3SA3', 'ITSA4', 'BBAS3', 'UGPA3', 'BRFS3'],
    '2019': ['ITUB4', 'VALE3', 'BBDC4', 'PETR4', 'ABEV3', 'BBAS3', 'B3SA3', 'ITSA4', 'LREN3', 'UGPA3']
}

# A partir de 2018-03-26 'BVMF3' passou a ser listada como 'B3SA3'
rename_stocks = {
    'BVMF3': 'B3SA3'
}
...
def gen_b3_cotahist_csv(b3_urls, csv_filename, include_list, rename_symbols=None):
    b3_cotahist_txts = read_b3_cotahist_zips(b3_urls)

    makedirs(path.dirname(csv_filename), exist_ok=True)
        ...
        for year, b3_cotahist_txt in b3_cotahist_txts.items():
            for ln, line in enumerate(StringIO(b3_cotahist_txt)):
                if ln > 0:
                    ...
                    if rename_symbols and symbol in rename_symbols:
                        symbol = rename_symbols[symbol]

                    if year in include_list and symbol in include_list[year]:
                        ...
                        writefile.write(
                            f"{symbol},{date},{price_open:.2f},{price_high:.2f},{price_low:.2f},{price_close:.2f},{volume}\n")
...
def main():
    ...
    start = timer()
    gen_b3_cotahist_csv(b3_cotahist_urls, csv_filename, include_list, rename_stocks)
